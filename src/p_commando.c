...
void CommandoStart(edict_t *ent)
{
	...
	i = ITEM_INDEX(item);
	ent->client->resp.inventory[i] = CLIP_MAGNUM_VALUE;

/*Savvy todo: add in magnum clips?
	item = FindItem("Magnum Clip");
	i = ITEM_INDEX(item);
	ent->client->resp.inventory[i] = 2; //number of magnum clips
//looks like shot_type is not part of inventory, ignore it
//	ent->client->resp.shot_type = i;
	ent->client->resp.inventory[ITEM_INDEX(FindAmmo(item))] = item->quantity; //ammo for mag clip
*/
