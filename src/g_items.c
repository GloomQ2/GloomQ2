...
/*Savvy proxy mine*/
#ifdef SAVVY_CONTENT
void Toss_Proxy_Mine (edict_t *ent, const gitem_t *item);
#endif
...
/*Savvy edited this function
grunts have 1 medpack, poisen wraiths pwn them :( poor grunts
so i made poisen merely removed, and 1/2 of medpack value used
*/

void Use_Health (edict_t *ent, const gitem_t *item) {
int quant = item->quantity;

	if(!ent->client->resp.inventory[ITEM_HEALTH]) { return; }
...
#ifdef MEDPACK_HALFED_BY_POISEN
	quant = (int)(quant/2); /*give 1/2 hp as normal from that medpack*/
#else
  quant = 0;
#endif
	}
	else if (ent->health >= ent->max_health && !(ent->client->resp.class_type == CLASS_GRUNT && ent->client->resp.upgrades & UPGRADE_STIMPACKS)) {
	return;
	}
	...
	if (ent->health < ent->max_health) {
		...
		if (ent->client->resp.class_type == CLASS_GRUNT && (ent->client->resp.upgrades & UPGRADE_STIMPACKS)) { ent->health += STIMPACK_HEALTH_AMOUNT; }
	}
}
...
/*Savvy
I placed definitions in balance.h
*/
...
/* pickup */	"Shell Clip",
/* width */		3,
/* Quantity */	CLIP_NORMAL_SHELLS_VALUE,
		CLIP_SHELLS,
		IT_CLIP,
		NULL,
		AMMO_SHELLS,
...
/* width */		3,
/* Quantity */	CLIP_EXPLOSIVE_SHELLS_VALUE,
		CLIP_SHELLS,
		IT_CLIP,
		NULL,
		AMMO_EXSHELLS,
...
/* width */		3,
/* Quantity */	CLIP_SCATTERSHOT_VALUE, // was 10
		CLIP_SCATTER,
		IT_CLIP,
		NULL,
		AMMO_SCATTER,
...
/* pickup */	"SMG Clip",
/* width */		3,
/* shots/clip */	CLIP_SUBMACHINEGUN_VALUE, // 80 in 1.3, 60 in 1.1d
		CLIP_SUB,
		IT_CLIP,
		NULL,
		AMMO_SUB,
...
#ifdef SAVVY_CONTENT
/*Savvy custom weapons, clips, items, etc*/
/*human mines*/
	{
		"item_pmine", 
		NULL,
...
/*Savvy edit, old values:
(ent, start, forward, 350, speed, 3 220)
ent start forward damage speed timer radius*/
	fire_grenade (ent, start, forward, FRAG_GRENADE_DAMAGE, speed, 3, FRAG_GRENADE_DAMAGE_RADIUS);
...
AngleVectors (angles, forward, NULL, NULL);
	...
	if(ent->client->ps.pmove.pm_flags & PMF_DUCKED) { speed = 100; }
	else { speed = 500; }
...
static void flare_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point) { BecomeExplosion1 (self); }
...
edict_t *tent = NULL;
	...
	while ((tent = findradius_c(tent,ent,FLARE_DETECTION_DISTANCE))) {
...
ent->nextthink = level.time + .2f;
	...
	if (ent->count++ > FLARE_TTL) { BecomeExplosion1 (ent); }
}
...
vec3_t dir;
edict_t *e, *first;
int activeflares, besttime, speed, max_active_flares;

max_active_flares = BIOTECH_AMMO_FLARES;
/*Savvy is a player throwing the flares?*/
  if (ent->client) {
  /*Savvy let's allow various limits for upgrades for different classes
  Savvy: fix, a hatchling upgraded with enhancement could have +2 max, because upgrades are binary, and reset for each class
         validating class through switch statement now*/
    switch (ent->client->resp.class_type) {
    case CLASS_BIO:
      if (ent->client->resp.upgrades & UPGRADE_BATTLETECH) { max_active_flares += BIOTECH_AMMO_BATTLETECH_FLARES_ADD; }
      if (ent->client->resp.upgrades & UPGRADE_FLARE_PACK) { max_active_flares += BIOTECH_FLARE_PACK_ADD; }
      break;
    }
  }
...
besttime = 0;
	...
	while ((e = G_Find3 (e, ENT_FLARE)) != NULL) {
		if (e->owner == ent) {
			...
			if (++activeflares == max_active_flares) { break; }
		}
	}

	if (activeflares == max_active_flares) { BecomeExplosion1 (first); }
...
/*was for guardian
ent->s.modelindex = 255;
ent->fly_sound_debounce_time = level.framenum + 20; //make visable
*/
...
fire_proxy_mine (ent, start, forward, speed, 30,100,0);
}
#endif
