...
edict_t	*t;
	...
	for ( t = g_edicts+1 ; t <= &g_edicts[globals.num_edicts] ; t++) {
		...
		continue;
		if (gi.pointcontents (t->s.origin) & CONTENTS_SOLID) {
			if (t->client) {
...
//		gi.dprintf ("WARNING: worldspawn without message field set\n");
	...
	coldblasta_index = gi.modelindex ("sprites/s_colda.sp2");
	coldblastb_index = gi.modelindex ("sprites/s_coldb.sp2");
#endif
