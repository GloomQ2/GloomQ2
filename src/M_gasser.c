...
void gasser_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf __attribute__((unused))) //FV7 attribute
{
	int damage = 4;
	...
	if (other->takedamage)
		T_Damage (other, self, self, vec3_origin, other->s.origin, vec3_origin, damage, 3, 0, MOD_OBSTACLE);
}
...
void gasser_use (edict_t *self, edict_t *other __attribute__((unused)), edict_t *activator __attribute__((unused))) //FV7 attributes
{
	if (!self->deadflag)
		gasser_fire_gas (self);
}

void gasser_die (edict_t *self, edict_t *inflictor __attribute__((unused)), edict_t *attacker, int damage, vec3_t point __attribute__((unused))) //FV7 attributes
{
	...
	if (!(self->spawnflags & 16384))
	{
		if (attacker->client && attacker->client->resp.team == TEAM_HUMAN)
			team_info.buildpool[TEAM_ALIEN] += COST_GASSER;
		else
...
void gasser_pain (edict_t *self, edict_t *other __attribute__((unused)), float kick __attribute__((unused)), int damage __attribute__((unused))) //FV7 attributes
{
