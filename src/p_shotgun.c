...
void ShotgunStart(edict_t *ent)
{
	...
	i = ITEM_INDEX(item);
	ent->client->resp.inventory[i] = EX_SHELL_CLIPS_START;

	item = FindItem("Smoke Grenade");
	i = ITEM_INDEX(item);
	ent->client->resp.inventory[i] = 3;/*2*/
...
i = ITEM_INDEX(item);
  ...
  (
	...
	ent->client->resp.inventory[ITEM_INDEX(FindItem("Smoke Grenade"))] == 3 && /*2*/
...
/*Savvy added this one in, if upgraded can replenish explosive shells*/
  if (ent->client->resp.upgrades & UPGRADE_EXPLOSIVE_SHELL_PACK) {
    if (ent->client->resp.inventory[i] < EX_SHELL_CLIPS_MAX) {
    item = FindItem("Shell Clip [EX]");
    i = ITEM_INDEX(item);
		wait += (EX_SHELL_CLIPS_MAX - ent->client->resp.inventory[i]) * 10;
    ent->client->resp.inventory[i] = EX_SHELL_CLIPS_MAX;
    }
  }
	...
	if (ent->client->resp.inventory[ITEM_INDEX(FindItem("Shells"))] != 0) {
		wait+=(3-ent->client->resp.inventory[i])*10;
		ent->client->resp.inventory[i] = 3;
	} else {
...
//		}
	...
	item = FindItem("Smoke Grenade");
	i = ITEM_INDEX(item);
	wait+=(3-ent->client->resp.inventory[i])*15;/*2-...*/
	ent->client->resp.inventory[i] = 3;/*2*/
	}
