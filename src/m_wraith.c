...
void WraithStart(edict_t *ent)
{
	...
	item = FindItem("Spit");
	i = ITEM_INDEX(item);
	ent->client->resp.inventory[i] = MAX_WRAITH_SPIT;
...
float wait = 0;
	...
	if (ent->client->healwait > level.time) {
		if (ent->client->resp.inventory[ent->client->ammo_index] == MAX_WRAITH_SPIT) { return 2; }
		else { return 0; }
	}
                             
	if (ent->client->resp.inventory[ent->client->ammo_index] == MAX_WRAITH_SPIT)
		return 2;

wait = (MAX_WRAITH_SPIT-ent->client->resp.inventory[ent->client->ammo_index]) * MAX_WRAITH_SPIT;
ent->client->resp.inventory[ent->client->ammo_index] = MAX_WRAITH_SPIT;
