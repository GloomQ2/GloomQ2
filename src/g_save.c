...
void EXPORT InitGame (void)
{
	...
	turret_mode = gi.cvar ("turret_mode", "3", 0);
	...
	spiker_max_spikes_per_client = gi.cvar ("spiker_max_spikes_per_client", "76", 0);
	spiker_spike_count = gi.cvar ("spiker_spike_count", "1", 0);
	spiker_damage = gi.cvar ("spiker_damage", "50", 0);
	spiker_damage_random = gi.cvar ("spiker_damage_random", "7", 0);
	spiker_speed = gi.cvar ("spiker_speed", "1100", 0);
	...
	voting =				gi.cvar ("voting",					"255",		CVAR_SERVERINFO);
...
//	speedtest =				gi.cvar ("speedtest",				"1",		CVAR_DEFAULT);
	...
	motd =					gi.cvar	("motd",					va("Welcome to Gloom %s",GLOOMVERSIONNO),			CVAR_DEFAULT);
	...
	suddendeath =			gi.cvar ("suddendeath",				"0",		CVAR_DEFAULT);
	overtime =				gi.cvar ("overtime",				"0",		CVAR_DEFAULT);
	...
	upgrades =				gi.cvar ("upgrades",				"1",		CVAR_DEFAULT);

#ifdef ADMINDLLHACK
	admindllbypass =		gi.cvar ("admindllbypass",			"1",		CVAR_LATCH | CVAR_NOSET);
	admindlltimer =			gi.cvar ("admindlltimer",			"0",		CVAR_LATCH | CVAR_NOSET);
#endif
...
#ifdef _WIN32
	gi.dprintf ("%s %s %s %s\n", GAMEVERSION, GLOOMVERSIONNO, __DATE__, __TIME__);
#else
	gi.dprintf ("%s %s %s\n",GAMEVERSION,GLOOMVERSIONNO,__DATE__);
#endif
	gi.cvar ("gamedate", __DATE__ , CVAR_SERVERINFO | CVAR_LATCH); // latch in baseq2
#ifdef ADMINDLLHACK
	if (admindllbypass->value) {
		admindll = fopen ("release/gamex86.dll", "r");
		if (admindll) {
			fclose (admindll);
			gi.cprintf (NULL, PRINT_HIGH, "WARNING: Server admin DLL detected, will attempt to evade.\n");
		}

		admindll2 = fopen ("release/gamex86.gloom-hax", "r");
		if (admindll2) {
			game.dllhack = true;
			fclose (admindll2);
			rename ("release/gamex86.gloom-hax", "release/gamex86.dll");
			gi.dprintf ("InitGame: Server admin DLL bypass complete!\n");
		}
	}
#endif
	...
	/*{
		...
		int i;
		unsigned char pad;
		FILE *temp = fopen ("c:/temp/foo.gif.enc", "rb");
		int len = fread (buff, 1, sizeof(buff), temp);

		fclose (temp);

		temp = fopen ("c:/temp/foo.gif.key", "rb");
		fread (buff2, 1, sizeof(buff2), temp);
		fclose (temp);

		temp = fopen("c:/temp/foo.gif", "wb");
		for (i = 0; i < len; i++) {
			pad = buff[i];
			pad ^= buff2[i];
			fwrite (&pad, sizeof(char), 1, temp);
		}
		fclose (temp);
		exit (1);
	}*/
	...
	//r1: win counter
	game.wins[TEAM_NONE] = game.wins[TEAM_HUMAN] = game.wins[TEAM_ALIEN] = 0;
