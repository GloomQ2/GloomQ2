...
void obstacle_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (other->takedamage)
		T_Damage (other, self, self, vec3_origin, other->s.origin, vec3_origin, damage, 0, 0, MOD_OBSTACLE);
}
...
void obstacle_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	if (!(self->spawnflags & 16384))
	{
		if (attacker->client && attacker->client->resp.team == TEAM_HUMAN)
			team_info.buildpool[TEAM_ALIEN] += COST_OBSTACLE;
		else
...
/*Savvy i butchered the function below so that i could set all heal rates in balance.h*/
void obstacle_think (edict_t *self) {
	if (self->monsterinfo.healtime < level.time) {
		if (self->health < self->max_health) {
		self->health += HEALRATE_OBSTACLE;/*10*/
			if (self->health > self->max_health) { self->health = self->max_health; }
		self->monsterinfo.healtime = level.time + 0.5f;
		}
    else {
		self->monsterinfo.healtime = level.time + 0.69f;
		self->health += HEALRATE_OBSTACLE_AFTER_MAX;/*1*/
			if (self->health > HEALRATE_OBSTACLE_HEALTH_CAP) { self->health = HEALRATE_OBSTACLE_HEALTH_CAP; } /*2500*/
		}
	}
}
