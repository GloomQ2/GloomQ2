...
void healer_touch(edict_t *self, edict_t *other, cplane_t *plane __attribute__((unused)), csurface_t *surf __attribute__((unused))) { //FV7 attributes
int damage = 5;

	if (self->target_ent) { self = self->target_ent; }
	...
	if (other->flags & FL_CLIPPING) {
		if (!VectorCompare (self->velocity, vec3_origin)) { other = self; }
		damage = 1000;
	...
	int c;
		if (level.time < other->wait) { return; }
/*SomeUnnamedCoder(R1?): hack because wait is used in menus and
makes it seem lagged/broken if client is
in menu when sitting on healer
Savvy: I remember this
*/
		if (level.framenum != (unsigned int)self->count) { //FV7 typecast int to unsigned int on self->count
		self->air_finished = self->delay;
		self->delay = 0;
		}
	self->delay ++;
	other->wait = level.time + 0.05f;
		if (random() * self->air_finished > 1) { return; }
	c = other->client->resp.class_type;
		if (self->enttype == ENT_EVIL_HEALER) {
			other->health -= 3;
			if (other->health <= 3) { T_Damage (other, self, self, vec3_origin, vec3_origin, vec3_origin, 3, 0, DAMAGE_NO_PROTECTION, MOD_HEALER); }
			else if (other->health <= 0) { gi.dprintf ("shouldn't be here\n"); }

			if (self->touch_debounce_time<level.time) {
				if (!(self->spawnflags & 64)) { gi.sound (other, CHAN_AUTO, SoundIndex (organ_healer1), 1, ATTN_NORM, 0); }
		...
		if (c == CLASS_BREEDER && other->client->build_timer > level.time) { other->client->build_timer -= 0.1; }
...
//#endif
		...
		if (!(self->spawnflags & HEAL_NO_HEALTH)) { other->health += 2; }
...
//#ifndef GUARDIAN_REGEN_NEAR_NEST
...
void healer_pain (edict_t *self __attribute__((unused)), edict_t *other __attribute__((unused)), float kick __attribute__((unused)), int damage __attribute__((unused))) //FV7 attributes
{
}
...
void healer_die (edict_t *self, edict_t *inflictor __attribute__((unused)), edict_t *attacker, int damage, vec3_t point __attribute__((unused))) //FV7 attributes
{
	...
	if (!(self->spawnflags & 16384))
	{
		if (attacker->client && attacker->client->resp.team == TEAM_HUMAN)
			team_info.buildpool[TEAM_ALIEN] += COST_HEALER;
		else
...
void healer_grow(edict_t *self, edict_t *other __attribute__((unused)), cplane_t *plane, csurface_t *surf __attribute__((unused))) //FV7 attributes
{
...
/*Savvy i added in the healrate crap, as well as a cap on health, since there really wasn't one for over healrate 1*/
void healer_think (edict_t *self) {
	if (self->health < self->max_health)
		self->health += HEALRATE_HEALER;
