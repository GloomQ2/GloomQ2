...
void HealbyBiotech(edict_t *ent, edict_t *bio)
{
	...
	if (ent->client->resp.inventory[ITEM_HEALTH] < numpacks && diffpacks > 0) {
		...
		gi.sound(ent,CHAN_AUTO,SoundIndex (items_s_health),1.0,ATTN_NORM,0);
	...
	if (bio->client->resp.upgrades & UPGRADE_GLOW_GRENADE)
	{
		if (diff == 0 && diffpacks == 0)
		{
			if (ent->health == ent->max_health)
			{
				...
				{
					gi.sound(ent,CHAN_AUTO,SoundIndex (items_m_health),1.0,ATTN_NORM,0);
...
#ifdef CACHE_CLIENTSOUNDS
	if (n==0)
		teamcastSound(TEAM_HUMAN, SoundIndex(base+n), 0.7);
	else if (n==1)
		teamcastSound(TEAM_HUMAN, SoundIndex(base+n), 1.0);
	else if (n>1 && n<7)
		gi.sound (self, CHAN_AUTO, SoundIndex(base+n), 1, ATTN_IDLE, 0);
#else
	switch(n)
	{
	case VOICE_SPAWN:
		teamcastSound(TEAM_HUMAN, gi.soundindex(va("voice/%s/spawn1.wav", s)), 0.7);
		break;
	case VOICE_HELP:
		teamcastSound(TEAM_HUMAN, gi.soundindex(va("voice/%s/help1.wav", s)), 1.0);
		break;
	case VOICE_ORDER:
		gi.sound (self, CHAN_AUTO, gi.soundindex(va("voice/%s/order1.wav", s)), 1, ATTN_IDLE, 0);
		break;
	case VOICE_AFFIRM:
		gi.sound (self, CHAN_AUTO, gi.soundindex(va("voice/%s/affirm1.wav", s)), 1, ATTN_IDLE, 0);
		break;
	case VOICE_DEFEND:
		gi.sound (self, CHAN_AUTO, gi.soundindex(va("voice/%s/defend1.wav", s)), 1, ATTN_IDLE, 0);
		break;
	case VOICE_NOAMMO:
		gi.sound (self, CHAN_AUTO, gi.soundindex(va("voice/%s/noammo1.wav", s)), 1, ATTN_IDLE, 0);
		break;
	case VOICE_TAUNT:
		gi.sound (self, CHAN_AUTO, gi.soundindex(va("voice/%s/taunt1.wav", s)), 1, ATTN_IDLE, 0);
		break;
	}
#endif
}
...
void AlienVoice (edict_t *ent, int n)
{
	...
	if (n >= 0 && n < 7)
		gi.sound (ent, CHAN_AUTO, SoundIndex (base+n), 1, ATTN_IDLE, 0);
#else

	if (n == 0){
		gi.sound (ent, CHAN_AUTO, gi.soundindex ("*scream.wav"), 1, ATTN_IDLE, 0);
	} else if (n > 0 && n < 7) {
		gi.sound (ent, CHAN_AUTO, gi.soundindex (va("*scream%d.wav", n)), 1, ATTN_IDLE, 0);
	}
#endif
}
...
const gclass_t classlist[] =
{
...

	{
		...
		100,
		TEAM_ALIEN,
		140, /*Savvy keep fov at 140, or you'll get screwed with hatchie look*/
...
/*			FRAME_stand01, FRAME_stand40, FRAME_run1, FRAME_run6,
	...
	},
	{
		...
		"models/objects/gibs/sm_meat/tris.md2",
		"models/objects/gibs/sm_meat/tris.md2",
		1.1,/*Savvy edit: was 1.2; 1.1 acts more like the old commando which would have to jump twice to get onto a crate*/
		false,
...
*/
	...
	/*	 Brian Added this	*/
	{
		...
		{16, 16, 16},
		8,
		100, //forever was 75, let's make wraiths more fun
		-60,
		200,
		TEAM_ALIEN,
		120,
