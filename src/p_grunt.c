...
void GruntVoice(edict_t *self, int n)
{
	...
	case VOICE_SPAWN:
		...
		teamcastSound(TEAM_HUMAN, SoundIndex(grunt_voice1+(randomMT()%3)), 0.7);
#else
		teamcastSound(TEAM_HUMAN, gi.soundindex(va("voice/grunt/spawn%i.wav", (randomMT()%3)+1)), 0.7);
#endif
		break;
	case VOICE_HELP:
#ifdef CACHE_CLIENTSOUNDS
		teamcastSound(TEAM_HUMAN, SoundIndex(grunt_voice4+(randomMT()&1)), 1.0);
#else
		teamcastSound(TEAM_HUMAN, gi.soundindex(va("voice/grunt/help%i.wav", (randomMT()&1)+1)), 1.0);
#endif
		break;
	case VOICE_ORDER:
#ifdef CACHE_CLIENTSOUNDS
		gi.sound (self, CHAN_AUTO, SoundIndex(grunt_voice6+(randomMT()%4)), 1, ATTN_NORM, 0);
#else
		gi.sound (self, CHAN_AUTO, gi.soundindex(va("voice/grunt/order%i.wav", (randomMT()%4)+1)), 1, ATTN_NORM, 0);
#endif
		break;
	case VOICE_AFFIRM:
#ifdef CACHE_CLIENTSOUNDS
		gi.sound (self, CHAN_AUTO, SoundIndex(grunt_voice10+(randomMT()%3)), 1, ATTN_NORM, 0);
#else
		gi.sound (self, CHAN_AUTO, gi.soundindex(va("voice/grunt/affirm%i.wav", (randomMT()%3)+1)), 1, ATTN_NORM, 0);
#endif
		break;
	case VOICE_DEFEND:
#ifdef CACHE_CLIENTSOUNDS
		gi.sound (self, CHAN_AUTO, SoundIndex(grunt_voice13+(randomMT()%3)), 1, ATTN_NORM, 0);
#else
		gi.sound (self, CHAN_AUTO, gi.soundindex(va("voice/grunt/defend%i.wav", (randomMT()%3)+1)), 1, ATTN_NORM, 0);
#endif
		break;
	case VOICE_NOAMMO:
#ifdef CACHE_CLIENTSOUNDS
		gi.sound (self, CHAN_AUTO, SoundIndex(grunt_voice16+(randomMT()%3)), 1, ATTN_NORM, 0);
#else
		gi.sound (self, CHAN_AUTO, gi.soundindex(va("voice/grunt/noammo%i.wav", (randomMT()%3)+1)), 1, ATTN_NORM, 0);
#endif
		break;
	case VOICE_TAUNT:
#ifdef CACHE_CLIENTSOUNDS
		gi.sound (self, CHAN_AUTO, SoundIndex(grunt_voice19+(randomMT()%3)), 1, ATTN_NORM, 0);
#else
		gi.sound (self, CHAN_AUTO, gi.soundindex(va("voice/grunt/taunt%i.wav", (randomMT()%3)+1)), 1, ATTN_NORM, 0);
#endif
		break;
	}
}
