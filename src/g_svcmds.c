...
/*Savvy added for server admins / rcon admins to give frags out
note: exact name match, ignores maximum and minimum
*/
void SV_Cmd_Givefrag_f (void) {
static char *syntax = "Usage: sv givefrag player\nGives a free frag to player\n";
char *name;
char *othername;
int i;
edict_t *ent;
qboolean found_player;
name = gi.argv(2);
found_player = false;
  if (gi.argc() < 3) { gi.cprintf(NULL,PRINT_HIGH,"%s", syntax); }
	for (i = 0; i < game.maxclients; i++) {
	ent = g_edicts + 1 + i;
		if ((ent->inuse) && (ent->client)) {
		othername = ent->client->pers.netname;
		  if (Q_stricmp(name, othername) == 0) {
      ent->client->resp.score++; /*increase by 1 frag*/
      gi.cprintf(NULL,PRINT_HIGH,"frag given to %s\n", name);
      gi.cprintf(ent,PRINT_HIGH,"Console gave you a frag! Thank the server admin!\n");
      found_player = true;
      break; /*should never have more than 1 name the same*/
      }
		}
	}
	if (found_player != true) { gi.cprintf(NULL,PRINT_HIGH,"Player \"%s\" not found in game, please check your spelling (it must be exact)\n"); }
}
void SV_Cmd_Takefrag_f (void) {
static char *syntax = "Usage: sv takefrag player\nTakes a frag away from player\n";
char *name;
char *othername;
int i;
edict_t *ent;
...
found_player = false;
  if (gi.argc() < 3) { gi.cprintf(NULL,PRINT_HIGH,"%s", syntax); }
	...
	ent = g_edicts + 1 + i;
		...
		othername = ent->client->pers.netname;
		  if (Q_stricmp(name, othername) == 0) {
      ent->client->resp.score--; /*decrease by 1 frag*/
      gi.cprintf(NULL,PRINT_HIGH,"frag taken from %s\n", name);
      gi.cprintf(ent,PRINT_HIGH,"Console took a frag from you!\n");
      found_player = true;
      break; /*should never have more than 1 name the same*/
      }
		}
...
void EXPORT ServerCommand (void)
{
  ...
  /*Savvy nice big fucking security risk R1
...
/*Savvy this is for give frag command*/
	else if (Q_stricmp(cmd, "givefrag") == 0) {
	SV_Cmd_Givefrag_f();
	}
/*Savvy this is for take frag command*/
	else if (Q_stricmp(cmd, "takefrag") == 0) {
	SV_Cmd_Takefrag_f();
	}
  else { gi.dprintf ("Unknown server command \"%s\"\n", cmd); }
}
