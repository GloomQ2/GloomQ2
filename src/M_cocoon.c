...
void cocoon_wait_large(edict_t *self)
{
	...
	if (!ceiling_eggs->value &&  !(self->spawnflags & 32))
...
void cocoon_die_large (edict_t *self, edict_t *inflictor __attribute__((unused)), edict_t *attacker, int damage, vec3_t point __attribute__((unused))) //FV7 attributes
{
	...
	if (attacker != self && (meansOfDeath != MOD_TRIGGER_HURT && meansOfDeath != MOD_CRUSH && meansOfDeath != MOD_LAVA)) {
		...
		// if death damage was under 30, regrow
		if (damage < 30 || meansOfDeath == MOD_PULSELASER)
		{
		...
		if ((self->flags & FL_MAP_SPAWNED) && level.time < (60*xmins->value) && meansOfDeath != MOD_OUT_OF_MAP) {
			self->health = self->max_health;
	...
	{
		gi.bprintf (PRINT_HIGH, "Spider spawn destroyed by %s!\n", attacker->client->pers.netname);
		...
		SendToTeam (TEAM_ALIEN, "Spider spawn destroyed!\n");
	...
	if (!(self->spawnflags & 16384))
	{
		if (attacker && attacker->client && attacker->client->resp.team == TEAM_HUMAN)
			team_info.buildpool[TEAM_ALIEN] += COST_EGG;
		else
...
void cocoon_pain (edict_t *self, edict_t *other __attribute__((unused)), float kick __attribute__((unused)), int damage __attribute__((unused))) //FV7 attributes
{
	if (ceiling_eggs->value || self->spawnflags & 32)
		self->gravity = -1;
...
void SP_monster_cocoon_use (edict_t *self, edict_t *other __attribute__((unused)), edict_t *activator __attribute__((unused))) //FV7 attributes
{
...
//	sound_pain  = SoundIndex (cocoon_drud1);
	...
	if (!self->enttype) {
		...
		//can't do this, affects spawning angles
...
void cocoon_touch(edict_t *self, edict_t *other, cplane_t *plane __attribute__((unused)), csurface_t *surf __attribute__((unused))) //FV7 attributes
{
	int damage = 10;
	edict_t *ent = NULL;
	...
	if (other->takedamage && damage) {
		T_Damage (other, self, self, vec3_origin, self->s.origin, vec3_origin, damage, 0, DAMAGE_NO_KNOCKBACK | DAMAGE_IGNORE_RESISTANCES, MOD_EGG);
	}
