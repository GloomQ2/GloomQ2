...
void C4_Explode_Medium_Smoke (edict_t* self)
{
	...
	if (self->think == C4_Explode_Medium) {
		...
		if (visible (self->owner, self) && self->owner && self->owner->owner)
...
//	vec3_t		v;
	...
	if (self->think == C4_Timer)
	{
		...
		if (!self->owner->inuse || (self->owner->client && self->owner->client->resp.team != TEAM_HUMAN && !(self->owner->client->pers.adminpermissions & PERMISSION_TOYS) ))
		{
			G_FreeEdict(self);
			return;
		}
...
/*Savvy cannot use this here easily: static void c4_touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf) {*/
...
/*Savvy make the parent ent unable to respawn (and get frags back to make another c4)*/
  if ((self->client) && (self->inuse)) { self->client->resp.can_respawn = false; }
