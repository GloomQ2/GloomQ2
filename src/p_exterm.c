...
void ExterStart(edict_t *ent)
{
	...
	i = ITEM_INDEX(item);
	ent->client->resp.inventory[i] = 1;/*2*/

	item = FindItem("Frag Grenade");
	i = ITEM_INDEX(item);
	ent->client->resp.inventory[i] = 4;/*4*/

/*Savvy 2 proxy mines to start with, add in healer functions later, once it is resolved that it works correctly*/
/*
	item = FindItem("Proxy Mine");
	i = ITEM_INDEX(item);
...
int ExterHeal (edict_t *ent, edict_t *healer)
{
	...
	if (ent->client->resp.inventory[ITEM_INDEX(FindItem("Frag Grenade"))] == 4 &&
		ent->client->resp.inventory[ITEM_INDEX(FindItem("Health"))] == 1 && /*2*/
