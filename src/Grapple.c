...
void CTFGrappleTouch (edict_t *self, edict_t *other, cplane_t *plane __attribute__((unused)), csurface_t *surf) //FV7 attribute
{
...
void CTFGrapplePull(edict_t *self)
{
	...
	if (self->spawnflags > CTF_GRAPPLE_STATE_FLY) {
		...
		if (self->spawnflags == CTF_GRAPPLE_STATE_PULL && vlen < 64) {
...
/*Savvy edited the speed crap to allow for custom upgrade*/
speed = CTF_GRAPPLE_SPEED;
  if ((ent->client) && (ent->client->resp.class_type) && (ent->client->resp.class_type == CLASS_HATCHLING) && (ent->client->resp.upgrades & UPGRADE_HATCHLING_ENHANCEMENT)) {
  speed *= HATCHLING_ENHANCEMENT_GRAPPLE_SPEED_MODIFIER;
  }
	
CTFFireGrapple (ent, start, forward, damage, speed);
